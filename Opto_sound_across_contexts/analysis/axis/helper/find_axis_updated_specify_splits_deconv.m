   ...
   for split = 1:splits
    ...
    %             [trials_ctrl] = size(ctrl_matrix, 1);
                ...
                for tr = trainA_ctrl_all;%train_trials_ctr %1:trials_ctrl
                    tr_ct = tr_ct +1;
                    post_rate = fs * nansum(data_matrix(tr, :, aframes), 3) ./ sum(~isnan(data_matrix(tr, :, aframes)), 3);
                    pre_rate  = fs * nansum(data_matrix(tr, :, bframes), 3) ./ sum(~isnan(data_matrix(tr, :, bframes)), 3);
                    
                    prepost_trial(tr,:,:) = [post_rate; pre_rate];   % [post, pre]
                end
                ...
                norm_sound_diff = sound_diff ./ (sqrt(sum(sound_diff.^2))+ eps); % Normalize sound axis
                ...
                norm_sound_post = post_sound ./ (sqrt(sum(post_sound.^2))+ eps); % Normalize sound axis
                ...
                norm_sound_pre = pre_sound ./ (sqrt(sum(pre_sound.^2))+ eps); % Normalize sound axis

                % --- 2) Calculate Stimulus + Sound Axis (stim + sound - sound) ---
                % stim + sound (rate per neuron)
                stim_rate = fs * nansum(stim_matrix(trainA_stim_all,:,aframes), 3) ...
                                ./ sum(~isnan(stim_matrix(trainA_stim_all,:,aframes)), 3);
                
                stim_sound_mean = nanmean(stim_rate, 1);
                % sound only (rate per neuron)
                sound_rate = fs * nansum(ctrl_matrix(trainA_ctrl_all,:,aframes), 3) ...
                                 ./ sum(~isnan(ctrl_matrix(trainA_ctrl_all,:,aframes)), 3);
                sound_mean = nanmean(sound_rate, 1);
                % axis
                stim_axis = stim_sound_mean - sound_mean;
                norm_stim_diff = stim_axis ./ (sqrt(sum(stim_axis.^2))+ eps); % Normalize stimulus axis

                %get just pre and post axis
                % post window
                stim_post = nanmean(fs*nansum(stim_matrix(trainA_stim_all,:,aframes),3)./sum(~isnan(stim_matrix(trainA_stim_all,:,aframes)),3), 1) ...
                          - nanmean(fs*nansum(ctrl_matrix(trainA_ctrl_all,:,aframes),3)./sum(~isnan(ctrl_matrix(trainA_ctrl_all,:,aframes)),3), 1);
                
                norm_stim_post = stim_post ./ (norm(stim_post) + eps);
                % pre window
                stim_pre = nanmean(fs*nansum(stim_matrix(trainA_stim_all,:,bframes),3)./sum(~isnan(stim_matrix(trainA_stim_all,:,bframes)),3), 1) ...
                         - nanmean(fs*nansum(ctrl_matrix(trainA_ctrl_all,:,bframes),3)./sum(~isnan(ctrl_matrix(trainA_ctrl_all,:,bframes)),3), 1);
                
                norm_stim_pre = stim_pre ./ (norm(stim_pre) + eps);
                norm_stim_pre = stim_pre ./ (norm(stim_pre) + eps);
                ...
                % CONCATENATE TRIALS ACROSS CONTROL AND STIM CONDITIONS (DECONV -> RATE)
                if ~isempty(bframes2)
                    % ---- active ----
                    X_act = [ ...
                        ctrl_matrix(ctrl_splits{current_dataset,1}(split).trainA, :, bframes2); ...
                        stim_matrix(stim_splits{current_dataset,1}(split).trainA, :, bframes2) ...
                    ];
                    act_rate = fs * nansum(X_act, 3) ./ sum(~isnan(X_act), 3);   % trials x neurons
                    act_mean = nanmean(act_rate, 1);                             % 1 x neurons
                
                    % ---- passive ----
                    X_pas = [ ...
                        ctrl_matrix(ctrl_splits{current_dataset,2}(split).trainA, :, bframes2); ...
                        stim_matrix(stim_splits{current_dataset,2}(split).trainA, :, bframes2) ...
                    ];
                    pas_rate = fs * nansum(X_pas, 3) ./ sum(~isnan(X_pas), 3);
                    pas_mean = nanmean(pas_rate, 1);
                
                    active_passive_sound_mean = [act_mean; pas_mean];
                
                else
                    % ---- active ----
                    X_act = [ ...
                        ctrl_matrix(ctrl_splits{current_dataset,1}(split).trainA, :, bframes); ...
                        stim_matrix(stim_splits{current_dataset,1}(split).trainA, :, bframes) ...
                    ];
                    act_rate = fs * nansum(X_act, 3) ./ sum(~isnan(X_act), 3);
                    act_mean = nanmean(act_rate, 1);
                
                    % ---- passive ----
                    X_pas = [ ...
                        ctrl_matrix(ctrl_splits{current_dataset,2}(split).trainA, :, bframes); ...
                        stim_matrix(stim_splits{current_dataset,2}(split).trainA, :, bframes) ...
                    ];
                    pas_rate = fs * nansum(X_pas, 3) ./ sum(~isnan(X_pas), 3);
                    pas_mean = nanmean(pas_rate, 1);
                ...
                norm_context_diff = context_axis ./ (norm(context_axis) + eps);
...
%                 norm_context_diff_stim = context_axis_stim ./ sqrt(sum(context_axis_stim.^2)); % Normalize context axis
                ...
                tim_matrix = [dff_st{1, current_dataset}.stim(:,mod_cells,:); dff_st{2, current_dataset}.stim(:,mod_cells,:)]; % stim + sound (active and passive)
                ctrl_matrix = [dff_st{1, current_dataset}.ctrl(:,mod_cells,:); dff_st{2, current_dataset}.ctrl(:,mod_cells,:)]; % sound only (active and passive)
                % (a) extract time window and reshape into trials x neurons
                stim_trial_neurons = fs*nansum(stim_matrix(trainA_stim_all,:,aframes),3)./sum(~isnan(stim_matrix(trainA_stim_all,:,aframes)),3);
                ctrl_trial_neurons = fs*nansum(ctrl_matrix(trainA_ctrl_all,:,aframes),3)./sum(~isnan(ctrl_matrix(trainA_ctrl_all,:,aframes)),3);
                ...
                %repeat for test
                stim_trial_neurons_test = fs*nansum(stim_matrix(test_stim_all,:,aframes),3)./sum(~isnan(stim_matrix(test_stim_all,:,aframes)),3);
                ctrl_trial_neurons_test = fs*nansum(ctrl_matrix(test_ctrl_all,:,aframes),3)./sum(~isnan(ctrl_matrix(test_ctrl_all,:,aframes)),3);
