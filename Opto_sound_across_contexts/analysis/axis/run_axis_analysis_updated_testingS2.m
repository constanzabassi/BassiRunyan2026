...
[proj,proj_ctrl,proj_norm,proj_norm_ctrl, weights,trial_corr_context,percent_correct,act,act_norm_ctrl,act_norm,percent_correct_concat,proj_concat,proj_concat_norm,engagement_concat,test_trials,test_trials_relative] = ...
    find_axis_updated_specify_splits(context_data.dff, choose_params, all_celltypes,[],split_params,{50:59,93:122,50:59,90:93});

save_dir = 'W:\Connie\results\Bassi2025\fig4\updated_4cv_combined_eng\S2_v2\';%'V:\Connie\results\opto_sound_2025\context\axis_lme_plots_updated\dff';
...
celltype = 4; %4 = all
plot_proj_meansplits_traces([1:24],proj_norm_ctrl, 'sound',celltype, [61:62],[0,0,0;.5,.5,.5],{'Active','Passive'},save_dir,'xlabel','Time from sound onset (s)','xlim',[31,122],'xticks',[31,61,91,121],'xticklabels',{-1 0 1 2});
plot_proj_meansplits_traces([1:24],proj_norm_ctrl, 'context',celltype, [61:62],[0,0,0;.5,.5,.5],{'Active','Passive'},save_dir,'xlabel','Time from stimulus onset (s)','xlim',[31,122],'xticks',[31,61,91,121],'xticklabels',{-1 0 1 2});
plot_proj_meansplits_traces([1:24],proj_norm, 'stim',celltype, [61:62],[0,0,0;.5,.5,.5],{'Active','Passive'},save_dir,'xlabel','Time from stim onset (s)','xlim',[31,122],'xticks',[31,61,91,121],'xticklabels',{-1 0 1 2});
