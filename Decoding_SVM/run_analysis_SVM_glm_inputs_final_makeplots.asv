...
for event = 1:length(event_names)
    ...
    plot_final_svm_traces_boxplots('full',event_names{event},do_passive,savepath,stim_ctrl_labels);
    ...
    plot_final_svm_traces_boxplots('nmatch',event_names{event},do_passive,savepath,stim_ctrl_labels);
...
for event = 1:length(event_names)
    ...
    plot_final_svm_traces_boxplots('nmatch',event_names{event},do_passive,savepath,stim_ctrl_labels); %plots the All population
    
    %comparing stim vs control!
    savepath = ['W:\Connie\results\Bassi2025\fig2\SVM_1\opto_ctrl'];
    plot_final_svm_traces_boxplots('stimctrl',event_names{event},do_passive,savepath,stim_ctrl_labels); %plots the All population
end
