...
%       - x_length is respected in data units.
        ...
        % --- Detect LabelAlignment in varargin ---
    align = 'center';  % default
    idx = find(strcmpi(varargin, 'LabelAlignment'));
    if ~isempty(idx)
    ...
    lineArgs = {};
    lineArgs = [lineArgs {'Color', p.Results.Color}];
    lineArgs = [lineArgs {'LineWidth', p.Results.LineWidth}];
    ...
    switch lower(align)
        case 'center'   % current behavior
            x_box = x_center_fig - box_width/2;
        case 'right'     % text starts at the center
            x_box = x_center_fig;
        case 'left'    % text ends at the center
            x_box = x_center_fig - box_width;
    end
