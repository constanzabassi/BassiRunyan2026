function place_text_labels(labels, colors, y_offset_base, fontSize,varargin)
    ...
    if size(varargin,2)>1 && ~isempty(varargin{1,2}) %adjust x offset independent of y
    ...
    if size(varargin,2) >= 3
        labelSpacing = varargin{3};
    else
        labelSpacing = 0.2;  % DEFAULT SPACING
    end
...
%     end
    ...
    y_offsets = y_direction * (0 : labelSpacing : labelSpacing*(num_labels-1));
%     y_offsets = linspace(0, 0.2 * (num_labels - 1), num_labels); % Adjusted scaling
    ...
    for i = 1:num_labels
        ...
        if size(varargin,2)>2
            text(text_x, text_y - y_offsets(i) * diff(y_range), labels{i}, ...
             'Color', colors(i,:), 'FontSize', fontSize,'BackgroundColor', 'none');   % white background);
        end
    end
end
