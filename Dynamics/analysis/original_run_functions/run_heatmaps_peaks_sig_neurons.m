plot_info = plotting_config(); %plotting params
load('V:\Connie\results\active\data_info\imaging_st.mat');
[imaging_st,info.eliminated_trials] = eliminate_trials(imaging_st,7,800);
imaging_passive = load('V:\Connie\results\passive\data_info\imaging_st.mat').imaging_st;

load('V:\Connie\results\active\data_info\info.mat');
load('V:\Connie\results\active\data_info\all_celltypes.mat');

alignment.conditions = [1,2]; %empty to run all conditions [5:8];
alignment.field_to_separate = {'is_stim_trial'}; %will separate into two parts control then stim

alignment.data_type = 'z_dff';% 'dff', 'z_dff', else it's deconvolved
alignment.type = 'all'; %'reward','turn','stimulus','ITI'
plot_info.min_max = [-0.25 1];%[0 .02];% %
alignment.number = [1:6]; %'reward','turn','stimulus'
alignment.cells = [cellfun(@(x) x.pyr_cells,all_celltypes,'UniformOutput',false);cellfun(@(x) x.som_cells,all_celltypes,'UniformOutput',false);cellfun(@(x) x.pv_cells,all_celltypes,'UniformOutput',false)];
alignment.title = {'PYR','SOM','PV'};
plot_info.xlabel = [];
plot_info.sorting_type = 1;
plot_info.ylabel = 'Frames';
plot_info.xlabel_events = {'sound','sound','sound','turn','reward','ITI'};
plot_info.colors_celltype = plot_info.colors_celltypes;
bin_size = 1;



save_path = 'W:/Connie/results/Bassi2025/fig3/avg_heatmap_across_entire_trial/';
% %%
% %plot heatmaps and grand avg for active context
% alignment.conditions = [1,2]; %empty to run all conditions [5:8];
% alignment.number = [1:6]; %'reward','turn','stimulus'
% alignment.type = 'all';
% mouse_data_conditions = heatmaps_avg_combined_selected_cells (imaging_st,plot_info,alignment,[],[save_path 'photostim'],bin_size, sig_mod_boot,0,1);
% 
% 
% % also make plot for passive context
% alignment.number = [1:3]; %'reward','turn','stimulus'
% alignment.type = 'stimulus';
% mouse_data_conditions_passive = heatmaps_avg_combined_selected_cells (imaging_passive,plot_info,alignment,[],[save_path  'photostim' '/passive/'],bin_size, sig_mod_boot,0,2);

%% HEATMAPS
%load sound modulated neurons
load('V:\Connie\results\opto_sound_2025\context\sounds\mod\prepost_sound\separate\mod_indexm.mat');
load('V:\Connie\results\opto_sound_2025\context\sounds\mod\prepost_sound\separate\sig_mod_boot.mat');
load('V:\Connie\results\opto_sound_2025\context\sounds\mod\prepost_sound\separate\sig_mod_boot_thr.mat');

% % Define the parameter sets
% mod_params.mod_threshold = 0.1;
% mod_params.chosen_mice = [1:25];
% param_sets = { 
%     struct('mod_threshold', mod_params.mod_threshold, 'threshold_single_side', 1, 'savestring', [ 'sound_positive_modulated'],'chosen_mice', mod_params.chosen_mice,'data_type','sounds'),
%     struct('mod_threshold', -1 * mod_params.mod_threshold, 'threshold_single_side', 1, 'savestring', [ 'sound_negative_modulated'],'chosen_mice', mod_params.chosen_mice,'data_type','sounds'),
%     struct('mod_threshold', mod_params.mod_threshold, 'threshold_single_side', 0, 'savestring', [ 'sound_all_modulated'],'chosen_mice', mod_params.chosen_mice,'data_type','sounds')
% };
% for i = 1:length(param_sets)
%         mod_params = param_sets{i};
%         [current_sig_cells] = get_thresholded_sig_cells_simple( mod_params, mod_indexm, sig_mod_boot_thr);
%         sig_cells = get_significant_neurons(current_sig_cells, mod_indexm, 'union'); %union of active and passive
%         cumsum(cellfun(@length, sig_cells))
%         
%         %do active
%         alignment.conditions = [1,2]; %empty to run all conditions [5:8];
%         alignment.number = [1:6]; %'reward','turn','stimulus'
%         alignment.type = 'all';
%         mouse_data_conditions = heatmaps_avg_combined_selected_cells (imaging_st,plot_info,alignment,[],[save_path mod_params.savestring ],bin_size, sig_cells,0,1);
% 
%         %do passive
%         alignment.number = [1:3]; %'reward','turn','stimulus'
%         alignment.type = 'stimulus';
%         mouse_data_conditions_passive = heatmaps_avg_combined_selected_cells (imaging_passive,plot_info,alignment,[],[save_path mod_params.savestring '/passive/'],bin_size, sig_cells,0,2);
% 
% 
% end
%% HEATMAPS SORTED BY SPONT!!
save_path = 'W:/Connie/results/Bassi2025/fig3/avg_heatmap_across_entire_trial_stretched/';
load('V:\Connie\results\opto_sound_2025\context\mod\prepost\separate\sig_mod_boot_thr.mat');% get spontaneously defined modulated neurons

load('V:\Connie\results\opto_sound_2025\context\data_info\context_data.mat');
% sorting_id_updated_datasets = get_sorting_indices_only(imaging_st, alignment, bin_size, sig_mod_boot, 0)
plot_info.position = [100,100,131,155];
sig_mod_boot = sig_mod_boot_thr(:,3);
[~,sorting_id_updated_datasets] = heatmaps_avg_combined_selected_cells_spont (context_data, plot_info,alignment,[],[save_path],1, sig_mod_boot,0);


%plot heatmaps and grand avg for active context
alignment.conditions = [1,2]; %empty to run all conditions [5:8];
alignment.number = [1:6]; %'reward','turn','stimulus'
alignment.type = 'all';        plot_info.position = [100,100,215,155];
mouse_data_conditions = heatmaps_avg_combined_selected_cells (imaging_st,plot_info,alignment,sorting_id_updated_datasets(1,:),[save_path 'photostim/sorted'],bin_size, sig_mod_boot,0,1);
alignment.number = [1:3]; %'reward','turn','stimulus'
alignment.type = 'stimulus';        plot_info.position = [100,100,112,155];
mouse_data_conditions_passive = heatmaps_avg_combined_selected_cells (imaging_passive,plot_info,alignment,sorting_id_updated_datasets(1,:),[save_path  'photostim' '/sorted/passive/'],bin_size, sig_mod_boot,0,2);

% 
%% now do SOUNDS!
load('V:\Connie\results\opto_sound_2025\context\sounds\mod\prepost_sound\separate\mod_indexm.mat');
load('V:\Connie\results\opto_sound_2025\context\sounds\mod\prepost_sound\separate\sig_mod_boot.mat');
load('V:\Connie\results\opto_sound_2025\context\sounds\mod\prepost_sound\separate\sig_mod_boot_thr.mat');

% Define the parameter sets
mod_params.mod_threshold = 0.1;
mod_params.chosen_mice = [1:25];
param_sets = { 
    struct('mod_threshold', mod_params.mod_threshold, 'threshold_single_side', 1, 'savestring', [ 'sound_positive_modulated'],'chosen_mice', mod_params.chosen_mice,'data_type','sounds'),
    struct('mod_threshold', -1 * mod_params.mod_threshold, 'threshold_single_side', 1, 'savestring', [ 'sound_negative_modulated'],'chosen_mice', mod_params.chosen_mice,'data_type','sounds'),
    struct('mod_threshold', mod_params.mod_threshold, 'threshold_single_side', 0, 'savestring', [ 'sound_all_modulated'],'chosen_mice', mod_params.chosen_mice,'data_type','sounds')
    };


for i = 1:length(param_sets)
i
    %get sorting index
    [current_sig_cells] = get_thresholded_sig_cells_simple( param_sets{i}, mod_indexm, sig_mod_boot_thr);
    sig_cells = get_significant_neurons(current_sig_cells, mod_indexm, 'union'); %union of active and passive
    cumsum(cellfun(@length, sig_cells))
    
    %do active
    alignment.conditions = [1,2]; %empty to run all conditions [5:8];
    alignment.number = [1:6]; %'reward','turn','stimulus'
    alignment.type = 'all';
    sorting_id_updated_datasets_sounds = get_sorting_indices_only(imaging_st, alignment, bin_size, sig_cells, 1)
%     [~,sorting_id_updated_datasets_sounds] = heatmaps_avg_combined_selected_cells (imaging_st,plot_info,alignment,[],[],bin_size, sig_cells,0,1);
%     [~,sorting_id_updated_datasets_sounds2] = heatmaps_avg_combined_selected_cells (imaging_st,plot_info,alignment,[],[],bin_size, sig_cells,0,1);
%     sound_sorting = [sorting_id_updated_datasets_sounds(2,:);sorting_id_updated_datasets_sounds2(2,:)];
    sound_sorting = [sorting_id_updated_datasets_sounds(2,:)];

        mod_params_plots = param_sets{i};
        [current_sig_cells] = get_thresholded_sig_cells_simple( mod_params_plots, mod_indexm, sig_mod_boot_thr);
        sig_cells = get_significant_neurons(current_sig_cells, mod_indexm, 'union'); %union of active and passive
        cumsum(cellfun(@length, sig_cells))
        
        %do active
        alignment.conditions = [1,2]; %empty to run all conditions [5:8];
        alignment.number = [1:6]; %'reward','turn','stimulus'
        alignment.type = 'all';
        plot_info.position = [100,100,215,155];
        heatmaps_avg_combined_selected_cells (imaging_st,plot_info,alignment,sound_sorting(1,:),[save_path mod_params_plots.savestring ],bin_size, sig_cells,0,1);

        %do passive
        alignment.number = [1:3]; %'reward','turn','stimulus'
        alignment.type = 'stimulus';
        plot_info.position = [100,100,112,155];
        heatmaps_avg_combined_selected_cells (imaging_passive,plot_info,alignment,sound_sorting(1,:),[save_path mod_params_plots.savestring '/passive/'],bin_size, sig_cells,0,2);
end

%% DYNAMICS PLOT OF CELLS- GET PEAKS!
plot_info.colors_celltype = [0.37 0.75 0.49 %light green
                            0.17 0.35 0.8  %blue
                            0.82 0.04 0.04 % red  
                            0 0 0.5]; %dark purple
dynamics_info.conditions = alignment.conditions; %[1,2]; %empty to run all conditions [5:8]; %1:8 for stim or empty to do all conditions!
dynamics_info.bin_size = 1;

%% make cdf plot using the peaks found -- ACTIVE
dynamics_info.bin_size = 1;
alignment.data_type = 'dff';% 'dff', 'z_dff', else it's deconvolved
alignment.conditions = [1,2]; %empty to run all conditions [5:8];
alignment.number = [1:6]; %'reward','turn','stimulus'
alignment.type = 'all';
[dynamics_info.max_cel_avg,dynamics_info.new_onsets,dynamics_info.binss,dynamics_info.original_onsets] = peak_times_avg_multiple_conditions (imaging_st,alignment,dynamics_info,1);

alignment.number = [1:3]; %'reward','turn','stimulus'
alignment.type = 'stimulus';
[dynamics_info_passive.max_cel_avg,dynamics_info_passive.new_onsets,dynamics_info_passive.binss,dynamics_info_passive.original_onsets] = peak_times_avg_multiple_conditions (imaging_passive,alignment,dynamics_info,2);

%% DO PHOTOSTIM FIRST
all_celltypes_updated = organize_significant_celltype_groups (all_celltypes,sig_mod_boot_thr(:,3)); %STIM 
for con = 1:2 %control and stim
    info.savepath = ['W:/Connie/results/Bassi2025/fig3/avg_heatmap_across_entire_trial/photostim/peaks' num2str(con)];
    dynamics_info.conditions = con;
    [dynamics_stats] =  cdf_peak_times_updated(squeeze(dynamics_info.max_cel_avg(con,1:24,:)),dynamics_info,all_celltypes_updated ,plot_info,info,1); %last number is number of nans wanted

    info.savepath = ['W:/Connie/results/Bassi2025/fig3/avg_heatmap_across_entire_trial/photostim/passive/peaks' num2str(con)];
    dynamics_info_passive.conditions = con;
    dynamics_info_passive.bin_size = dynamics_info.bin_size;
    [dynamics_stats] =  cdf_peak_times_updated(squeeze(dynamics_info_passive.max_cel_avg(con,1:24,:)),dynamics_info_passive,all_celltypes_updated ,plot_info,info,1); %last number is number of nans wanted

end

%% DO SOUNDS
for i = 1:length(param_sets)
        mod_params = param_sets{i};
        [current_sig_cells] = get_thresholded_sig_cells_simple( mod_params, mod_indexm, sig_mod_boot);
        sig_cells = get_significant_neurons(current_sig_cells, mod_indexm, 'union'); %union of active and passive
        all_celltypes_updated = organize_significant_celltype_groups (all_celltypes,sig_cells); 
        for con = 1:2 %control and stim
            %do active
            info.savepath = ['W:/Connie/results/Bassi2025/fig3/avg_heatmap_across_entire_trial/' mod_params.savestring '/peaks' num2str(con)];
            dynamics_info.conditions = con;
            [dynamics_stats] =  cdf_peak_times_updated(squeeze(dynamics_info.max_cel_avg(con,1:25,:)),dynamics_info,all_celltypes_updated ,plot_info,info,1); %last number is number of nans wanted
            
            %do passive
            info.savepath = ['W:/Connie/results/Bassi2025/fig3/avg_heatmap_across_entire_trial/' mod_params.savestring '/passive/peaks' num2str(con)];
            dynamics_info_passive.conditions = con;
            dynamics_info_passive.bin_size = dynamics_info.bin_size;
            [dynamics_stats] =  cdf_peak_times_updated(squeeze(dynamics_info_passive.max_cel_avg(con,1:25,:)),dynamics_info_passive,all_celltypes_updated ,plot_info,info,1); %last number is number of nans wanted
        end
end

%%
for i = 1:length(param_sets)
i
    %get sorting index
    [current_sig_cells] = get_thresholded_sig_cells_simple( param_sets{i}, mod_indexm, sig_mod_boot_thr);
    sig_cells = get_significant_neurons(current_sig_cells, mod_indexm, 'union'); %union of active and passive
    total_n = cumsum(cellfun(@length, sig_cells))
    [total_sig_cells,total_sig_cells_per_dataset,sig_cells_permouse] = find_sig_celltypes_ns(sig_cells,all_celltypes);
    table_sound_heatmaps1 = struct2table_recursive(total_sig_cells,param_sets{i}.savestring);
    table_sound_heatmaps2 = struct2table_recursive(total_sig_cells_per_dataset,param_sets{i}.savestring);
    table_sound_heatmaps = [table_sound_heatmaps1;table_sound_heatmaps2];
    save(fullfile(save_path, strcat('table_',param_sets{i}.savestring,'.mat')), 'table_sound_heatmaps');
    writetable(table_sound_heatmaps, fullfile(save_path, strcat('table_sound_heatmaps_',param_sets{i}.savestring,'.csv')));
end