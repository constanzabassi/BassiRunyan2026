...
if ~isempty(alignment.conditions) && length(alignment.conditions) >= 1
    for con = 1:length(alignment.conditions)
        ...
        set(f,'Units','points','Position',plot_info.position); %[100 100 170 216]
        ...
        for ce = 1:ncelltypes
            ...
            % -------- make plot --------
            %%% CHANGED: use fixed axis, not nexttile
            ax = ax_all(ce);
            axes(ax);       % make it current for helpers that use gca
            hold(ax,'on');
            ...
            if isempty(event_onsets)
                alignment_event_onset = 1;
            else
                alignment_event_onset = adjusted_onsets;
            end

            %%% CHANGED: removed duplicate nexttile(ce) here
            % ax = nexttile(ce);

            if isempty(sorting_id)
                plot_heatmap_celltype(ax, data_to_plot, plot_info, ...
                    sorting_id_updated, alignment_event_onset, adjusted_onsets);
            else
                plot_heatmap_celltype(ax, data_to_plot, plot_info, ...
                    sorting_id{ce}, alignment_event_onset, adjusted_onsets);
            end
            ...
            %%% CHANGED: apply to the correct axis, not gca
            set(ax, 'box','off','xtick',[]);

            % add color bar
            
            if active_passive == 2
                cb = add_skinny_colorbar(ax, 6, [], 0.3);
...
%                 cb = add_skinny_colorbar(ax, 6, 0.3, 0.16, 0.06);
            ...
            ylabel(ax, alignment.title{ce}, 'FontSize', 7);
        ...
        %%% CHANGED: use fixed 4th axis instead of nexttile(4)
        ax = ax_all(4);
        axes(ax);
        ...
        for ce = 1:ncelltypes
            mean_mouse_data_celltypes{ce} = compute_avg_across_neurons_and_alignment( ...
                imaging_st, sig_mod_boot, con, alignment, {alignment.cells{ce,:}}, active_passive, avg_across_datasets);
        end
        
        if size(mean_mouse_data_celltypes{1},2) > nan_insert_positions
            xlims = [1, size(mean_mouse_data_celltypes{1},2) + length(nan_insert_positions)];
        else
            xlims = [1, size(mean_mouse_data_celltypes{1},2)];
        end
        ...
        if strcmp(all_conditions{1, 3},'Control') && con == 1
            set(ax, 'xtick', adjusted_event_onsets, ...
                    'xticklabel', plot_info.xlabel_events, ...
                    'xticklabelrotation',45);
        else
            set(ax, 'xtick', adjusted_event_onsets, ...
                    'xticklabel', plot_info.xlabel_events_stim_sound, ...
                    'xticklabelrotation',45);
        end
        
        if active_passive == 2
            addScaleBar(ax, 30, "1 s",[],[],'LabelAlignment','center');
        else
            addScaleBar(ax, 30, "1 s");
        end
        ...
        if ~isempty(save_data_directory)
            ...
            exportgraphics(f, fullfile(save_data_directory, [image_string '.pdf']), 'ContentType','vector');
        end
    end
end
