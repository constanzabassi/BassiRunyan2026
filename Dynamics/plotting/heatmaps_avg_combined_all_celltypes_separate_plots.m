...
set(figure(90),'Units','inches', 'Position', [1,1,3,2.4])
...
%         mouse_data_conditions{m} = mouse_data{m};
        ...
        % Add colorbar to the *current axis*, and make it skinny
            ...
            cb_pos(1) = cb_pos(1) + 0.11;
            cb_pos(2) = cb_pos(2) + 0.1;
...
set(figure(90),'Units','inches', 'Position', [1,1,2.8,1.9])
...
%     plot(squeeze(mean(binned_data_all(:,ce,:),1)),'LineWidth',1.5,'color',plot_info.colors_celltype(ce,:));
    ...
    for i = 1:length(adjusted_event_onsets)
        xline(adjusted_event_onsets(i),'--k','LineWidth',.5)
    end

    for i = 1:length(nan_insert_positions)
        for n = 1:num_nans
            xline(nan_insert_positions(i)+n-1,'-w','LineWidth',.5);
        end
...
if ~isempty(save_data_directory)
    ...
    image_string = strcat('avgtrace_condition_',num2str(alignment.conditions,3),'_type_',alignment.data_type);
    saveas(91,fullfile([ save_data_directory '/' image_string '_datasets.svg']));
    saveas(91,fullfile([ save_data_directory '/' image_string '_datasets.fig']));
    exportgraphics(figure(91),fullfile([save_data_directory '/' image_string '_datasets.pdf']), 'ContentType', 'vector');

    %adjust sizing
        image_string = strcat('heatmaps_condition_',num2str(alignment.conditions,3));
%     set(figure(90),'Units','inches', 'Position', [1,1,2.2,1.9])
    figure(90)
%     adjusted_event_onsets(4) = adjusted_event_onsets(4)-5;
    set(gca,'xtick',adjusted_event_onsets,'xticklabel',plot_info.xlabel_events,'xticklabelrotation',45);
    set(figure(90),'Units','inches', 'Position', [1,1,2.3,1.9])
    exportgraphics(figure(90),fullfile([save_data_directory '/' image_string 'sizing2_datasets.pdf']), 'ContentType', 'vector');
    
    image_string = strcat('avgtrace_condition_',num2str(alignment.conditions,3),'_type_',alignment.data_type);
%     set(gca, 'FontSize', 7, 'Units', 'inches', 'Position', [1,1,1.75,1.2]);
    figure(91)
    set(gca, 'FontSize', 7, 'Units', 'inches', 'Position', [1,1,1.55,1.2]);
    exportgraphics(figure(91),fullfile([save_data_directory '/' image_string 'sizing2_datasets.pdf']), 'ContentType', 'vector');
